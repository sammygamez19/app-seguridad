...
{if(aRespuestas[k][m]["correcta"]==1)
{correccion[k]="MAL";break;}}}
testrespondida=Array();exito=Array();for(m=0;m<aRespuestas[k].length;m++)
{if(m<10)var mx="00"+m;else if(m<100)var mx="0"+m;else var mx=m;if($("#resp"+kx+"_"+mx).is(':checked'))
{testrespondida.push($("#respC"+kx+"_"+mx)[0].innerHTML);if(aRespuestas[k][m]["correcta"]==1){exito.push(1);}
else{exito.push(0);}}
...
for(x=0;x<testrespondida.length;x++){responderMultipleXAPI(correcta,respuesta,testrespondida[x],exito[x]);}}
...
completarXapi(tipoFinal,tiempoReg);puntosReg=parseInt(porcentajeAcierto);$("#numPuntos").html(parseInt(porcentajeAcierto));$("#btnFinalizar").unbind("click");$("#btnAtras").unbind("click");$("#btnAdelante").unbind("click");cargarPantallaFinal(tipoFinal);}
...
numBtn=0;cadenaBtn="000";$("#indica").html((numBtn+1)+"/"+numPreguntas);$("#avance").css("width",(numBtn+1)*100/numPreguntas+"%");$("#numTest"+cadenaBtn).html((numBtn+1)+".");$(".preguntas").hide();$("#pregunta"+cadenaBtn).show();setTimeout(function(){if($("#pistaAudio"+cadenaBtn).length!=0)$('#btnAudio'+cadenaBtn).click();},500);$("#btnAtras").unbind("click").click(function(e){e.preventDefault();cambiarPregunta("AT");});$("#btnAdelante").unbind("click").click(function(e){e.preventDefault();cambiarPregunta("AD");});$("#btnAtras").addClass("disable");$("#btnAdelante").removeClass("disable").addClass("btn-primary");};function inicializarVariablesXapi(){}
